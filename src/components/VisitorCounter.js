...
export default function VisitorCounter() {
    ...
    return (
        <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] md:text-xs">
            <span className="flex h-1.5 w-1.5 rounded-full bg-blue-400 group-hover:bg-blue-300 transition-colors animate-pulse" />
            <span className="text-gray-400">
                Total Visitors: <span className="text-blue-400 font-bold tracking-wider">{count.toLocaleString()}</span>
            </span>
        </div>
    );
}
